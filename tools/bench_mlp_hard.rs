...
fn main() {
    ...
    let steps: usize = args.get(1).and_then(|s| s.parse().ok()).unwrap_or(6);
    let batch: usize = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(32);

    // Harder benchmark: 1024 -> 4096 -> 1024 (8,388,608 params)
    let in_dim = 1024usize;
    let hidden = 4096usize;
    let out_dim = 1024usize;
    let params = in_dim * hidden + hidden * out_dim;
    println!(
        "bench-mlp-hard steps={steps} batch={batch} dims={in_dim}->{hidden}->{out_dim} params={params}"
    );

    let j = jules_train_like(steps, batch, in_dim, hidden, out_dim);
    ...
    match python_numpy_baseline(steps, batch, in_dim, hidden, out_dim) {
        Ok(py) => {
            let py_sec_per_step = py.0 / py.3.max(1) as f64;
            println!(
                "Python baseline: {:.3}s total ({} steps), {:.6}s/step, {:.2} steps/s, checksum={:.4}",
                py.0, py.3, py_sec_per_step, py.1, py.2
            );
            println!(
...
fn jules_train_like(
    ...
    let mut w1 = Tensor::from_data(vec![in_dim, hidden], init_vec(in_dim * hidden, 11));
    let mut w2 = Tensor::from_data(vec![hidden, out_dim], init_vec(hidden * out_dim, 22));
    let x = Tensor::from_data(vec![batch, in_dim], init_vec(batch * in_dim, 33));
    ...
    for _ in 0..steps {
        ...
        checksum += y.sum_all() * 1e-10;
        w1.scale_inplace(0.99998);
        w2.scale_inplace(0.99998);
    }
    let dt = t0.elapsed().as_secs_f64();
    (dt, steps as f64 / dt.max(1e-9), checksum)
}

fn python_numpy_baseline(
    steps: usize,
    batch: usize,
    in_dim: usize,
    hidden: usize,
    out_dim: usize,
) -> Result<(f64, f64, f32, usize), String> {
    let script = r#"
import sys, time
try:
    import numpy as np
...
w1 = init_vec(in_dim * hidden, 11).reshape(in_dim, hidden)
w2 = init_vec(hidden * out_dim, 22).reshape(hidden, out_dim)
x = init_vec(batch * in_dim, 33).reshape(batch, in_dim)
...
w1 = init_vec(in_dim * hidden, 11)
w2 = init_vec(hidden * out_dim, 22)
x = init_vec(batch * in_dim, 33)
...
for _ in range(steps):
    ...
    checksum += sum(y) * 1e-10
    for i in range(len(w1)): w1[i] *= 0.99998
    for i in range(len(w2)): w2[i] *= 0.99998
