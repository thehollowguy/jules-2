...
use crate::ast::{
    AccessMode, AgentDecl, BinOpKind, Block, ComponentDecl, ComponentAccess,
    DimExpr, ElemType, EnumDecl, Expr, FnDecl, GenericParam, Item,
    ModelDecl, ModelLayer, Param, Pattern, Program, StructDecl, StructField,
    SystemDecl, TrainDecl, Type, UnOpKind, VecFamily, VecSize,
};
...
pub enum Ty {
    ...
    // ── Tensors (Feature 1) ───────────────────────────────────────────────────
    /// `tensor<elem>[d0, d1, …]`
    Tensor { elem: ElemType, shape: Vec<Dim> },

    // ── SIMD vector / matrix types (Feature 2) ────────────────────────────────
    Vec   { size: VecSize, family: VecFamily },
    Mat   { size: VecSize },   // always float
    Quat,
...
impl Ty {
    ...
    pub fn is_tensor_like(&self) -> bool {
        matches!(self, Ty::Tensor { .. } | Ty::Vec { .. } | Ty::Mat { .. } | Ty::Quat)
    }
    ...
    pub fn display(&self) -> String {
        match self {
            ...
            Ty::Tensor { elem, shape } => {
                let dims: Vec<_> = shape.iter().map(|d| match d {
            ...
            Ty::Ref { mutable, inner } => {
                if *mutable { format!("&mut {}", inner.display()) }
                else        { format!("&{}", inner.display()) }
            }
...
impl Diagnostic {
    pub fn error(span: Span, message: impl Into<String>) -> Self {
        Diagnostic { severity: Severity::Error, span, message: message.into(), notes: vec![] }
    }
    pub fn warning(span: Span, message: impl Into<String>) -> Self {
        Diagnostic { severity: Severity::Warning, span, message: message.into(), notes: vec![] }
    }
    pub fn note(span: Span, message: impl Into<String>) -> Self {
        Diagnostic { severity: Severity::Note, span, message: message.into(), notes: vec![] }
    }
...
impl InferCtx {
    ...
    pub fn unify(&mut self, a: &Ty, b: &Ty) -> bool {
        ...
        match (&a, &b) {
            ...
            (Ty::Tensor { elem: ea, shape: sa }, Ty::Tensor { elem: eb, shape: sb }) => {
                ea == eb && sa.len() == sb.len() && sa.iter().zip(sb).all(|(a, b)| {
                    match (a, b) {
            ...
            (Ty::Tuple(ts_a), Ty::Tuple(ts_b)) => {
                ts_a.len() == ts_b.len()
                    && ts_a.iter().zip(ts_b).all(|(a, b)| self.unify(a, b))
            }
...
impl TypeCk {
    ...
    fn collect_item(&mut self, item: &Item) {
        match item {
            ...
            Item::Mod { items: Some(inner), .. } => {
                for i in inner { self.collect_item(i); }
            }
            _ => {}
        }
    }
    ...
    fn collect_struct(&mut self, s: &StructDecl, is_component: bool) {
        let fields = s.fields.iter()
            ...
            .collect();
        self.symbols.structs.insert(s.name.clone(), StructInfo {
            name: s.name.clone(),
            fields,
            is_component,
        });
    ...
    fn collect_component(&mut self, c: &ComponentDecl) {
        let fields = c.fields.iter()
            .map(|f| (f.name.clone(), self.lower_ast_type(&f.ty, f.span)))
            .collect();
        self.symbols.structs.insert(c.name.clone(), StructInfo {
            name: c.name.clone(),
            fields,
            is_component: true,
        });
    ...
    fn collect_fn(&mut self, f: &FnDecl) {
        let params = f.params.iter().map(|p| {
            let ty = p.ty.as_ref().map(|t| self.lower_ast_type(t, p.span))
        ...
        let ret = f.ret_ty.as_ref()
            ...
            .unwrap_or(Ty::Unit);
        self.symbols.fns.insert(f.name.clone(), FnInfo {
            name:     f.name.clone(),
            params,
            ret,
    ...
    pub fn lower_ast_type(&mut self, ty: &Type, span: Span) -> Ty {
        match ty {
            ...
            Type::Tensor { elem, shape } => {
                ...
                Ty::Tensor { elem: elem.clone(), shape: dims }
            }

            Type::Vec { size, family } =>
                Ty::Vec { size: *size, family: *family },
            ...
            Type::Tuple(ts) => {
                Ty::Tuple(ts.iter().map(|t| self.lower_ast_type(t, span)).collect())
            }

            Type::Array { elem, len } => {
                // Try to evaluate the length expression.
                if let Some(n) = eval_const_expr(len) {
                    Ty::Array { elem: Box::new(self.lower_ast_type(elem, span)), len: n }
                } else {
                    self.diag.error(span,
            ...
            Type::Slice(inner) =>
                Ty::Slice(Box::new(self.lower_ast_type(inner, span))),
            ...
            Type::Option(inner) =>
                Ty::Option(Box::new(self.lower_ast_type(inner, span))),

            Type::FnPtr { params, ret } => {
                let ps = params.iter().map(|t| self.lower_ast_type(t, span)).collect();
                let r  = self.lower_ast_type(ret, span);
                Ty::FnPtr { params: ps, ret: Box::new(r) }
            }
        }
    }
    ...
    fn lower_dim(&self, d: &DimExpr) -> Dim {
        match d {
            ...
            DimExpr::Expr(e)  => eval_const_expr(e)
                .map(Dim::Lit)
    ...
    pub fn check_program(&mut self, program: &Program) {
        ...
        let systems: Vec<_> = program.items.iter().filter_map(|i| {
            if let Item::System(s) = i { Some(s) } else { None }
        }).collect();
    ...
    fn check_item(&mut self, item: &Item) {
        match item {
            ...
            Item::Mod { items: Some(inner), .. } => {
                for i in inner { self.check_item(i); }
            }
            _ => {}
        }
    }
        ...
        for param in &f.params {
            let ty = match &param.ty {
                ...
                None => {
                    ...
                    self.diag.warning(param.span,
                        format!("parameter `{}` has no type annotation; inferred as `_`",
                                param.name));
                    self.infer.fresh()
                }
            };
    ...
    fn check_system(&mut self, s: &SystemDecl) {
        ...
        for param in &s.params {
            if param.ty.is_none() {
                self.diag.error(param.span, format!(
        ...
        if let Some(q) = &s.explicit_query {
            for comp in q.with.iter().chain(q.without.iter()) {
                if !self.symbols.structs.contains_key(comp.as_str()) {
                    self.diag.error(q.span, format!(
                        "query references unknown component `{}`", comp
                    ));
                }
            }
        }
            ...
            for j in (i + 1)..systems.len() {
                ...
                for acc_a in &a.accesses {
                    for acc_b in &b.accesses {
                        ...
                        {
                            self.diag.push(
                                Diagnostic::warning(a.span, format!(
    ...
    pub fn check_expr(&mut self, expr: &Expr, env: &mut TyEnv) -> Ty {
        match expr {
            ...
            Expr::Path { span, segments } => {
                ...
                match env.lookup(&name) {
                    ...
                    None => {
                        ...
                        if let Some(fi) = self.symbols.fns.get(segments.last().map(|s| s.as_str()).unwrap_or("")) {
                            Ty::FnPtr {
            ...
            Expr::VecCtor { span, size, elems } => {
                ...
                for e in elems {
                    ...
                    if !ty.is_float_scalar() {
                        self.diag.warning(e.span(), format!(
                            "vec constructor element should be f32; got `{}`", ty.display()
                        ));
                    }
                }
                Ty::Vec { size: *size, family: VecFamily::Float }
            }

            // ── Array literal ──────────────────────────────────────────────────
            Expr::ArrayLit { span, elems } => {
                if elems.is_empty() {
                    return Ty::Array { elem: Box::new(self.infer.fresh()), len: 0 };
                }
                ...
                for e in &elems[1..] {
                    ...
                    if !self.infer.unify(&first_ty, &ty) {
                        self.diag.error(*span, format!(
                            ...
                            first_ty.display(), ty.display()
                        ));
                    }
                }
                Ty::Array { elem: Box::new(first_ty), len: elems.len() as u64 }
            }

            // ── Binary operators ───────────────────────────────────────────────
            Expr::BinOp { span, op, lhs, rhs } =>
                self.check_binop(*span, *op, lhs, rhs, env),
                ...
                match op {
                    ...
                    UnOpKind::Not => {
                        if !matches!(ty, Ty::Bool) && !ty.is_numeric() {
                            self.diag.error(*span, format!(
                                "unary `!`/`~` requires `bool` or integer, got `{}`", ty.display()
                            ));
                        }
                        if matches!(ty, Ty::Bool) { Ty::Bool } else { ty }
                    }
                    UnOpKind::Deref => {
                        match ty {
                            Ty::Ref { inner, .. } => *inner,
                            _ => {
                                self.diag.error(*span, format!(
                                    "cannot dereference `{}`", ty.display()
                                ));
                                self.infer.fresh()
                            }
                        }
                    }
                    UnOpKind::Ref    => Ty::Ref { mutable: false, inner: Box::new(ty) },
                    UnOpKind::RefMut => Ty::Ref { mutable: true,  inner: Box::new(ty) },
                }
            }

            // ── Assignment ────────────────────────────────────────────────────
            Expr::Assign { span, op: _, target, value } => {
            ...
            // ── Field access ───────────────────────────────────────────────────
            Expr::Field { span, object, field } =>
                self.check_field_access(*span, object, field, env),

            // ── Index ──────────────────────────────────────────────────────────
            Expr::Index { span, object, indices } => {
                let obj_ty = self.check_expr(object, env);
                for idx in indices {
                    let i_ty = self.check_expr(idx, env);
                    if !matches!(i_ty,
                        Ty::Scalar(ElemType::I32 | ElemType::I64 | ElemType::U32 | ElemType::U64 | ElemType::Usize)
                    ) {
                        self.diag.warning(*span, format!(
                            "index should be an integer type, got `{}`", i_ty.display()
                        ));
                    }
                }
                match &obj_ty {
                    ...
                    _ => {
                        self.diag.error(*span, format!(
                            "cannot index into `{}`", obj_ty.display()
                        ));
                        self.infer.fresh()
                    }
                }
            ...
            Expr::MatMul { span, lhs, rhs } =>
                self.check_matmul(*span, lhs, rhs, env),
            ...
            Expr::Pow { span, base, exp } => {
                ...
                if !b_ty.is_numeric() && !b_ty.is_tensor_like() {
                    self.diag.error(*span, format!(
                        "`**` base must be numeric or tensor, got `{}`", b_ty.display()
                    ));
                }
                if !e_ty.is_numeric() {
                    self.diag.error(*span, format!(
                        "`**` exponent must be numeric, got `{}`", e_ty.display()
                    ));
                }
                b_ty
            }
            ...
            Expr::IfExpr { span, cond, then, else_ } => {
                ...
                if !matches!(cond_ty, Ty::Bool) {
                    self.diag.error(*span, format!(
                        "`if` condition must be `bool`, got `{}`", cond_ty.display()
                    ));
                }
            ...
            // ── Closure ────────────────────────────────────────────────────────
            Expr::Closure { span: _, params, ret_ty, body } => {
                env.push_scope();
                    ...
                    for (fname, fexpr) in fields {
                        ...
                        if let Some(expected) = info.field_ty(fname) {
                            ...
                            if !self.infer.unify(&expected, &expr_ty) {
                                self.diag.error(fexpr.span(), format!(
                                    "field `{}` of `{}` expects `{}`, got `{}`",
                                    fname, name, expected.display(), expr_ty.display()
                                ));
                            }
                        } else {
                            self.diag.error(*span, format!(
                                "`{}` has no field `{}`", name, fname
                            ));
                        }
                    }
                    if is_component { Ty::Component(name.clone()) } else { Ty::Struct(name.clone()) }
                } else {
                    self.diag.error(*span, format!(
                        "unknown struct `{}`", name
                    ));
    ...
    fn check_binop(
        ...
        match op {
            ...
            | BinOpKind::Div | BinOpKind::Rem | BinOpKind::FloorDiv => {
                ...
                // vec3 × mat3 → vec3
                if let Ty::Vec { size: vs, family: VecFamily::Float } = &l {
                    if let Ty::Mat { size: ms } = &r {
                        if vs == ms {
                            return Ty::Vec { size: *vs, family: VecFamily::Float };
                        }
                    }
                }
                ...
                if let (Ty::Tensor { .. }, ref scalar) | (ref scalar, Ty::Tensor { .. }) = (&l, &r) {
                    if scalar.is_numeric() {
                        return if matches!(&l, Ty::Tensor { .. }) { l.clone() } else { r.clone() };
                    }
                }
                // Plain numeric
                if !self.infer.unify(&l, &r) {
                    self.diag.error(span, format!(
                        ...
                        l.display(), format!("{:?}", op).to_lowercase(), r.display()
                    ));
                }
                l
            }
    ...
    fn check_matmul(
        ...
        if let (Ty::Tensor { elem: ea, shape: sa }, Ty::Tensor { elem: eb, shape: sb }) = (&l, &r) {
            ...
            return Ty::Tensor { elem: ea.clone(), shape: result_shape };
        }
    ...
    fn check_field_access(
        ...
        match &resolved {
            ...
            Ty::Vec { size, family } => {
                ...
                if field.chars().all(|c| valid.contains(&c)) {
                    match field.len() {
                        ...
                        2 => Ty::Vec { size: VecSize::N2, family: *family },
                        3 => Ty::Vec { size: VecSize::N3, family: *family },
                        4 => Ty::Vec { size: VecSize::N4, family: *family },
                        _ => {
                    ...
                    self.diag.error(span, format!(
                        "`{}` has no field `{}`", resolved.display(), field
                    ));
                    self.infer.fresh()
                }
            }
            ...
            Ty::Tuple(ts) => {
                if let Ok(idx) = field.parse::<usize>() {
                    if idx < ts.len() {
                        ...
                        self.diag.error(span, format!(
                            "tuple index {} out of range (len {})", idx, ts.len()
                        ));
                        self.infer.fresh()
                    }
                } else {
                    self.diag.error(span, format!(
                        "invalid tuple field `{}`", field
                    ));
                    self.infer.fresh()
                }
            }
            ...
            _ => {
                self.diag.error(span, format!(
                    "type `{}` has no field `{}`", resolved.display(), field
                ));
                self.infer.fresh()
            }
        }
                ...
                for (i, (expected, actual)) in params.iter().zip(arg_tys.iter()).enumerate() {
                    if !self.infer.unify(expected, actual) {
                        self.diag.error(span, format!(
                            ...
                            i + 1, expected.display(), actual.display()
                        ));
                    }
                }
                *ret.clone()
                ...
                if let Expr::Ident { name, .. } = func {
                    if let Some(fi) = self.symbols.fns.get(name.as_str()).cloned() {
                        if fi.params.len() != arg_tys.len() {
                            self.diag.error(span, format!(
                                "function `{}` expects {} argument(s), {} provided",
                                name, fi.params.len(), arg_tys.len()
                            ));
                        }
    ...
    fn check_method_call(
        ...
        match (&recv_ty, method) {
            (Ty::Tensor { elem, shape }, "transpose") => {
                ...
                Ty::Tensor { elem: elem.clone(), shape: new_shape }
            }
            (Ty::Tensor { elem, .. }, "reshape") => {
                // Return a dynamic tensor of the same element type.
                Ty::Tensor { elem: elem.clone(), shape: vec![Dim::Dynamic] }
            }
            (Ty::Tensor { elem, .. }, "sum" | "mean" | "max" | "min") => {
                Ty::Scalar(elem.clone())
            }
            (Ty::Tensor { elem, shape }, "softmax" | "relu" | "sigmoid" | "tanh") => {
                Ty::Tensor { elem: elem.clone(), shape: shape.clone() }
            }
            (Ty::Vec { .. }, "dot") => Ty::Scalar(ElemType::F32),
            (Ty::Vec { .. }, "normalize") => recv_ty.clone(),
            (Ty::Vec { .. }, "length" | "magnitude") => Ty::Scalar(ElemType::F32),
            (Ty::Vec { size: VecSize::N3, .. }, "cross") => recv_ty.clone(),
    ...
    fn check_stmt(&mut self, stmt: &crate::ast::Stmt, env: &mut TyEnv) {
        use crate::ast::Stmt;
        match stmt {
            Stmt::Let { span, pattern, ty, init, .. } => {
                let declared = ty.as_ref()
                ...
                if !self.infer.unify(&declared, &init_ty) {
                    self.diag.error(*span, format!(
                        ...
                        declared.display(), init_ty.display()
                    ));
                }
            ...
            Stmt::Return { span, value } => {
                if let Some(e) = value { self.check_expr(e, env); }
                else { let _ = *span; }
            }
            ...
            Stmt::EntityFor { span, var, query, body, .. } => {
                ...
                if let Some(f) = &query.filter {
                    ...
                    if !matches!(ft, Ty::Bool) {
                        self.diag.error(*span,
                            "entity query filter must be a `bool` expression");
                    }
                }
                env.push_scope();
            ...
            Stmt::While { span: _, cond, body, .. } => {
                let ct = self.check_expr(cond, env);
                if !matches!(ct, Ty::Bool) {
                    self.diag.error(cond.span(), format!(
                        "`while` condition must be `bool`, got `{}`", ct.display()
                    ));
                }
                self.check_block(body, env);
            }

            Stmt::Loop { body, .. } => { self.check_block(body, env); }

            Stmt::If { span, cond, then, else_, .. } => {
                let ct = self.check_expr(cond, env);
                if !matches!(ct, Ty::Bool) {
                    self.diag.error(*span, format!(
                        "`if` condition must be `bool`, got `{}`", ct.display()
                    ));
                }
                ...
                if let Some(e) = else_ {
                    match e.as_ref() {
                        ...
                        crate::ast::IfOrBlock::Block(b) => { self.check_block(b, env); }
                    }
                }
            }

            Stmt::Match { span: _, expr, arms } => {
                ...
                for arm in arms {
                    ...
                    if let Some(ref prev) = arm_ty {
                        if !self.infer.unify(prev, &bt) {
                            self.diag.error(arm.span, format!(
                                "match arm type `{}` is incompatible with `{}`",
                                bt.display(), prev.display()
                            ));
                        }
                    } else {
                        arm_ty = Some(bt);
            ...
            Stmt::Spawn(sb) => { self.check_block(&sb.body, env); }
            Stmt::Sync(sb)  => { self.check_block(&sb.body, env); }
            Stmt::Atomic(ab)=> { self.check_block(&ab.body, env); }
        }
    }
    ...
    fn bind_pattern(&mut self, pat: &Pattern, ty: &Ty, env: &mut TyEnv) {
        match pat {
            ...
            Pattern::Lit(..) | Pattern::Range { .. } | Pattern::Or { .. } | Pattern::Enum { .. } => {}
        }
    }
    ...
    fn expect_ty(&mut self, expected: &Ty, actual: &Ty, span: Span, ctx: &str) {
        if !self.infer.unify(expected, actual) {
            self.diag.error(span, format!(
                ...
                ctx, expected.display(), actual.display()
            ));
        }
    }
    ...
    fn check_model(&mut self, m: &ModelDecl) {
        ...
        for layer in &m.layers {
            if let ModelLayer::Dropout { span, rate } = layer {
                if *rate < 0.0 || *rate >= 1.0 {
                    self.diag.error(*span, format!(
                        "dropout rate {} is out of range [0.0, 1.0)", rate
                    ));
                }
            }
            if let ModelLayer::Attention { span, num_heads, head_dim } = layer {
                if *num_heads == 0 || *head_dim == 0 {
                    self.diag.error(*span,
                        "attention layer must have num_heads > 0 and head_dim > 0");
                }
            }
            if let ModelLayer::Embed { span, vocab_size, embed_dim } = layer {
        ...
        for layer in &m.layers {
            match layer {
                ...
                ModelLayer::Pool { span, size_h, size_w, .. } => {
                    if *size_h == 0 || *size_w == 0 {
                        self.diag.error(*span, "pool size must be > 0");
                    }
                }
                ModelLayer::Conv2d { span, filters, kernel_h, kernel_w, .. } => {
                ...
                ModelLayer::SubModel { span, name } => {
                    if !self.symbols.models.contains_key(name.as_str()) {
                        self.diag.error(*span, format!(
                            "model layer references unknown model `{}`", name
                        ));
                    }
                }
                _ => {}
    ...
    fn check_train(&mut self, t: &TrainDecl) {
        ...
        if !self.symbols.agents.contains_key(&t.agent) {
            self.diag.error(t.span, format!(
                "`train` references unknown agent `{}`", t.agent
            ));
        }
        ...
        if let Some(model_name) = &t.model {
            if !self.symbols.models.contains_key(model_name.as_str()) {
                self.diag.error(t.span, format!(
                    "`train` references unknown model `{}`", model_name
                ));
            }
        }
        ...
        for sig in &t.signals {
            if sig.weight < 0.0 {
                self.diag.error(sig.span, format!(
        ...
        for (name, expr) in &t.hyper {
            ...
            if !ty.is_float_scalar() && !matches!(ty, Ty::Scalar(_)) {
                self.diag.warning(expr.span(), format!(
    ...
    fn check_agent(&mut self, a: &AgentDecl) {
            ...
            match &ls.kind {
                crate::ast::LearningKind::Reinforcement | crate::ast::LearningKind::Imitation => {
                    if ls.policy_model.is_none() {
                        self.diag.warning(ls.span, format!(
                        ...
                        if !self.symbols.models.contains_key(pm.as_str()) {
                            self.diag.error(ls.span, format!(
        ...
        if a.behaviors.is_empty() {
            self.diag.warning(a.span, format!(
...
mod tests {
    ...
    fn bool_ty() -> Type { Type::Scalar(ElemType::Bool) }
    ...
    fn float_lit(v: f64) -> Expr { Expr::FloatLit { span: dummy(), value: v } }
    fn int_lit(v: u128)  -> Expr { Expr::IntLit   { span: dummy(), value: v } }
    fn bool_lit(v: bool) -> Expr { Expr::BoolLit  { span: dummy(), value: v } }
    fn ident(name: &str) -> Expr { Expr::Ident    { span: dummy(), name: name.into() } }

    fn make_checker() -> TypeCk { TypeCk::new() }
    ...
    fn test_matmul_shape_correct() {
        ...
        let mut ck  = make_checker();
        let mut env = TyEnv::new();
        env.bind("A", Ty::Tensor {
            elem:  ElemType::F32,
            shape: vec![Dim::Lit(8), Dim::Lit(16)],
        });
        env.bind("B", Ty::Tensor {
            elem:  ElemType::F32,
    ...
    fn test_matmul_shape_mismatch_error() {
        ...
        assert!(ck.diag.has_errors(), "expected error for inner dim mismatch");
    }
    ...
    fn test_hadamard_same_shape_ok() {
        let mut ck  = make_checker();
        let mut env = TyEnv::new();
        let t = Ty::Tensor { elem: ElemType::F32, shape: vec![Dim::Lit(4)] };
        env.bind("A", t.clone());
        env.bind("B", t);
    ...
    fn test_tensor_concat_axis0_shape() {
        ...
        let mut env = TyEnv::new();
        env.bind("A", Ty::Tensor {
            elem: ElemType::F32, shape: vec![Dim::Lit(3), Dim::Lit(8)] });
        env.bind("B", Ty::Tensor {
            elem: ElemType::F32, shape: vec![Dim::Lit(5), Dim::Lit(8)] });
    ...
    fn test_tensor_concat_inner_mismatch_error() {
        ...
        env.bind("A", Ty::Tensor {
            elem: ElemType::F32, shape: vec![Dim::Lit(3), Dim::Lit(8)] });
        env.bind("B", Ty::Tensor {
            elem: ElemType::F32, shape: vec![Dim::Lit(5), Dim::Lit(4)] });
        let expr = Expr::TensorConcat {
            span: dummy(),
            lhs:  Box::new(ident("A")),
            rhs:  Box::new(ident("B")),
    ...
    fn test_vec3_ctor_ok() {
        ...
        let ty = ck.check_expr(&expr, &mut env);
        assert!(!ck.diag.has_errors(), "{:?}", ck.diag.items);
        assert_eq!(ty, Ty::Vec { size: VecSize::N3, family: VecFamily::Float });
    }
    ...
    fn test_vec3_field_x_is_scalar() {
        let mut ck  = make_checker();
        let mut env = TyEnv::new();
        env.bind("v", Ty::Vec { size: VecSize::N3, family: VecFamily::Float });
        let expr = Expr::Field {
            span:   dummy(),
    ...
    fn test_model_dropout_out_of_range_error() {
        let m = ModelDecl {
            ...
            attrs:     vec![],
            name:      "Bad".into(),
            layers:    vec![ModelLayer::Dropout { span: dummy(), rate: 1.5 }],
    ...
    fn test_model_dropout_valid() {
        let m = ModelDecl {
            ...
            attrs:     vec![],
            name:      "Good".into(),
            layers:    vec![ModelLayer::Dropout { span: dummy(), rate: 0.1 }],
            device:    ModelDevice::Auto,
            optimizer: None,
        };
        let mut ck = make_checker();
    ...
    fn test_model_attention_zero_heads_error() {
        let m = ModelDecl {
            ...
            attrs:     vec![],
            name:      "Attn".into(),
            layers:    vec![ModelLayer::Attention { span: dummy(), num_heads: 0, head_dim: 64 }],
            device:    ModelDevice::Auto,
            optimizer: None,
        };
        let mut ck = make_checker();
    ...
    fn test_system_unannotated_param_error() {
        let s = SystemDecl::new(
            dummy(),
            "Update",
            vec![Param { span: dummy(), name: "dt".into(), ty: None, default: None, mutable: false }],
            Block::new(dummy()),
        );
        let mut ck = make_checker();
        ck.check_system(&s);
    ...
    fn test_struct_field_access_ok() {
        let mut ck = make_checker();
        ck.symbols.structs.insert("Pos".into(), StructInfo {
            name:   "Pos".into(),
    ...
    fn test_diagnostic_note_attached() {
        let d = Diagnostic::error(dummy(), "oops")
            .with_note(dummy(), "defined here");
